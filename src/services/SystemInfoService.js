...
class SystemInfoService {
  static execPromise(cmd) {
    return PlatformService.execPromise(cmd);
  }
    ...
    try {
      ...
      try {
        ...
        try {
          const cpus = os.cpus();
          if (cpus && cpus.length) {
            modelName = cpus[0].model || modelName;
            cores = cpus.length;
          }
        } catch {}
      }
  ...
  static async getUptimeInfo() {
    try {
      ...
      try {
        ...
        try {
          seconds = Math.floor(os.uptime() || 0);
        } catch {}
      }
  ...
  static async getPlatformInfo() {
    try {
      ...
      return { os: os.type(), release: os.release(), arch: os.arch() };
    }
  }
  ...
  static async getServerResources(serverId) {
    try {
      ...
      return await PlatformService.getProcessResources(server ? server.pid : null);
    } catch {
